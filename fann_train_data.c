											   ...
											   float desired_error)
{
	...
	buf = (char *) calloc(TRAIN_STR_LEN,sizeof(char));
	...
	{
		sprintf(buf,"Max epochs %8d. Desired error: %.10f.\n", max_epochs, desired_error);
		R_ShowMessage(buf);
	}
	...
	for(i = 1; i <= max_epochs; i++)
	{
			...
			desired_error_reached == 0))
		{
			...
			{
				sprintf(buf,"Epochs     %8d. Current error: %.10f. Bit fail %d.\n", i, error,
					   ann->num_bit_fail);
				R_ShowMessage(buf);
			}
...
FANN_EXTERNAL void FANN_API fann_train_on_file(struct fann *ann, const char *filename,
	   unsigned int max_epochs, unsigned int epochs_between_reports, float desired_error)
{
