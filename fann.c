...
if(ann->network_type == FANN_NETTYPE_SHORTCUT)
{
	sprintf(buf,"  Hidden layer                       :%4d neurons, 0 bias\n",
   layer_it->last_neuron - layer_it->first_neuron);
	R_ShowMessage(buf);
}
else
{
	sprintf(buf,"  Hidden layer                       :%4d neurons, 1 bias\n",
   layer_it->last_neuron - layer_it->first_neuron - 1);
	...
	R_ShowMessage(buf);
#ifdef FIXEDFANN
	sprintf(buf,"Decimal point                        :%4d\n", ann->decimal_point);
	R_ShowMessage(buf);
	sprintf(buf,"Multiplier                           :%4d\n", ann->multiplier);
	R_ShowMessage(buf);
#else
