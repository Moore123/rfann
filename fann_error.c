...
char *buf;
	...
	{
		buf = (char *) calloc(FANN_ERR_LEN,sizeof(char));
		sprintf(buf, "FANN Error %d: %s", errdat->errno_f, errdat->errstr);
		R_ShowMessage(buf);
		free(buf);
	}
}
...
void fann_error(struct fann_error *errdat, const enum fann_errno_enum errno_f, ...)
{
	...
	else
	{
		...
		if(errstr == NULL)
		{
			R_ShowMessage("Unable to allocate memory.");
			return;
		}
	}
	...
	{
		buf = (char *) calloc(FANN_ERR_LEN,sizeof(char));
		sprintf(buf, "FANN Error %d: %s", errno_f, errstr);
		R_ShowMessage(buf);
		free(buf);
	}
